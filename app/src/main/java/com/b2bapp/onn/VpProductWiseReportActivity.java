...
public class VpProductWiseReportActivity extends AppCompatActivity implements View.OnClickListener {

    private ImageView imgBack;
    private EditText etStartDate, etEndDate;// etProductStyle;
    private Spinner spinCollection, spinAreas, spinState, productStyleSp;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        vp = pref.getStringPreference(VpProductWiseReportActivity.this,Preferences.User_fname)
                + " "+ pref.getStringPreference(VpProductWiseReportActivity.this,Preferences.User_lname);
        ...
        spinCollectionAdapter = new SpinCollectionAdapter(collectionModelArrayList,VpProductWiseReportActivity.this);
        ...
        etStartDate.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                new DatePickerDialog(VpProductWiseReportActivity.this,date,myCalendar.get(Calendar.YEAR),myCalendar.get(Calendar.MONTH),myCalendar.get(Calendar.DAY_OF_MONTH)).show();
            }
        });
        etEndDate.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                new DatePickerDialog(VpProductWiseReportActivity.this,date1,myCalendar.get(Calendar.YEAR),myCalendar.get(Calendar.MONTH),myCalendar.get(Calendar.DAY_OF_MONTH)).show();
            }
        });

        if (GlobalVariable.vp_report_date_from.equals("")){
            etStartDate.setText(start);
        }else{
            etStartDate.setText(GlobalVariable.vp_report_date_from);
        }

        if (GlobalVariable.vp_report_date_to.equals("")){
            etEndDate.setText(end);
        }else{
            etEndDate.setText(GlobalVariable.vp_report_date_to);
        }

        //etProductStyle.setText(GlobalVariable.vp_report_style_no);

        areaAdapter = new AreaAdapter(areaModelArrayList, VpProductWiseReportActivity.this);
        spinAreas.setAdapter(areaAdapter);

        stateAdapter = new AreaAdapter(stateModelArrayList, VpProductWiseReportActivity.this);
        spinState.setAdapter(stateAdapter);

        spinState.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
                selected_state = stateModelArrayList.get(i).getName();
                getAreaTask(selected_state);
            }
    ...
    public void getCollectionWiseProductStyles(String collectionId) {
        dialogView.showCustomSpinProgress(VpProductWiseReportActivity.this);
                        ...
                        try {
                            ...
                            if (!response.getBoolean("error")) {
                                ...
                                if (searchCollectionModelArrayList.size() > 0) {
                                    ...
                                    ArrayAdapter<String> spinnerPhoneArrayAdapter2 = new ArrayAdapter<String>(VpProductWiseReportActivity.this, R.layout.spinner_item, products);
                        ...
                        try {
                            ...
                            if (!response.getBoolean("error")) {
                                ...
                                for (int i=0;i<collectionModelArrayList.size();i++){
                                    if (collectionModelArrayList.get(i).getId().equals(GlobalVariable.vp_report_collection)){
    ...
    public void getStateTask(){
        ...
        JsonObjectRequest jsonObjReq = new JsonObjectRequest(Request.Method.GET,
                WebServices.URL_VP_WISE_STATES+"?vp="+vp, new JSONObject(postParam),
                new Response.Listener<JSONObject>() {
                    @Override
                    public void onResponse(JSONObject response) {
                        ...
                        try {
                            if (!response.getBoolean("error")) {
                                ...
                                stateModelArrayList.add(categoryModel1);*/
                                ...
                                if (categoriesArray.length() > 0) {
                                    for (int i = 0; i < categoriesArray.length(); i++) {
                                        ...
                                        categoryModel.setId("");
                                        categoryModel.setName(categoryObj.getString("state"));
                                        categoryModel.setState("");

                                        stateModelArrayList.add(categoryModel);
                                    }
                                }
                                ...
                                for (int i=0;i<stateModelArrayList.size();i++){
                                    if (stateModelArrayList.get(i).getName().equals(GlobalVariable.vp_report_state)){
                                        getAreaTask(stateModelArrayList.get(i).getName());
    ...
    public void getAreaTask(String state){
        //dialogView.showCustomSpinProgress(RsmTeamWiseReportActivity.this);
        Map<String, String> postParam= new HashMap<String, String>();

        Log.d("area url>>",WebServices.URL_VP_WISE_AREAS+"?vp="+vp+"&state="+state);
        JsonObjectRequest jsonObjReq = new JsonObjectRequest(Request.Method.GET,
                WebServices.URL_VP_WISE_AREAS+"?vp="+vp+"&state="+state, new JSONObject(postParam),
                new Response.Listener<JSONObject>() {
                    @Override
                    public void onResponse(JSONObject response) {
                        Log.d(TAG+"->areas>>", response.toString());
    ...
    public void submitReportData(){
        GlobalVariable.vp_report_date_from = etStartDate.getText().toString();
        GlobalVariable.vp_report_date_to = etEndDate.getText().toString();
        GlobalVariable.vp_report_collection = selected_collection;
        GlobalVariable.vp_report_area = selected_area;
        GlobalVariable.vp_report_state = selected_state;
        //GlobalVariable.vp_report_style_no = etProductStyle.getText().toString();

        startActivity(new Intent(VpProductWiseReportActivity.this, VpProductWiseReportResultActivity.class));
    ...
    public void onBackPressed() {
        super.onBackPressed();
        Intent intent = new Intent(VpProductWiseReportActivity.this, VpStartingActivity.class);
